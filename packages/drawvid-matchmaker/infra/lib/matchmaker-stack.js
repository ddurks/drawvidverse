...
class MatchmakerStack extends cdk.Stack {
    constructor(scope, id, gameKey = 'cyberia', props) {
        super(scope, id, props);
        // Load game configuration
        const gameConfigPath = (0, path_1.join)(__dirname, '../../../..', 'games', `${gameKey}.config.json`);
        const gameConfig = JSON.parse((0, fs_1.readFileSync)(gameConfigPath, 'utf-8'));
        // DynamoDB Table
        // ========================================================================
        ...
        // ========================================================================
        const worldServerTargetGroup = new elbv2.NetworkTargetGroup(this, 'WorldServerTargetGroup', {
            vpc,
            port: 7777,
        ...
        cleanupHandler.addToRolePolicy(new iam.PolicyStatement({
            actions: [
                'ecs:StopTask',
                'ecs:DescribeTasks',
            ],
        ...
        const certificate = acm.Certificate.fromCertificateArn(this, 'DrawvidWildcardCert', 'arn:aws:acm:us-east-2:593615615124:certificate/a5b44baf-5840-40d5-8167-46417bf29d77');
        ...
        new route53.ARecord(this, 'MatchmakerDomainRecord', {
            ...
            target: route53.RecordTarget.fromAlias(new route53Targets.ApiGatewayv2DomainProperties(matchmakerDomain.regionalDomainName, matchmakerDomain.regionalHostedZoneId)),
        });
        // ========================================================================
        // Network Load Balancer for World Servers
        // ========================================================================
        const worldServerNLB = new elbv2.NetworkLoadBalancer(this, 'WorldServerNLB', {
            vpc,
            internetFacing: true,
        ...
        worldServerNLB.addListener('WorldServerListener', {
            ...
            certificates: [
                elbv2.ListenerCertificate.fromArn('arn:aws:acm:us-east-2:593615615124:certificate/a5b44baf-5840-40d5-8167-46417bf29d77'),
            ],
        ...
        new route53.ARecord(this, 'WorldServerARecord', {
            zone: hostedZone,
            recordName: 'world',
            target: route53.RecordTarget.fromAlias(new route53Targets.LoadBalancerTarget(worldServerNLB)),
        });
